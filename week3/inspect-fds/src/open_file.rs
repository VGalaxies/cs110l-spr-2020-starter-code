...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let path_name = fs::read_link(fd_path).ok()?;
        let name = OpenFile::path_to_name(path_name.to_str()?);

        let fdinfo_path_str = format!("/proc/{}/fdinfo/{}", pid, fd);
        let fdinfo_path = Path::new(&fdinfo_path_str);
        ...
        return Some(OpenFile::new(name, cursor, access_mode))
    }
